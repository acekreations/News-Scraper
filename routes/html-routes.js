...
module.exports = function(app, db){
  ...
  app.get("/note/:id", function(req, res){
    db.Articles.findOne({_id: req.params.id}).populate("note").then(function(articlesDB){
      res.json(articlesDB);
    }).catch(function(err){
      console.log(err);
    });
  ...
  app.post("/save/:id", function(req, res){
    db.Notes.create(req.body).then(function(notesDB){
      console.log(notesDB);
      db.Articles.findOneAndUpdate({_id: req.params.id}, { note: notesDB._id }, {new: true}).then(function(articlesDB){
        res.json(articlesDB);
      }).catch(function(err){
        console.log(err);
      });
